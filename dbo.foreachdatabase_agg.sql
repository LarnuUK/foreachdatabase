CREATE OR ALTER PROC [dbo].[foreachdatabase_agg] @Command nvarchar(MAX),
...
AS BEGIN
    ...
    DECLARE @Delimiter nchar(4) = @CRLF + @CRLF;
    SET @Command_Run = @Command_Run + @CRLF + @CRLF +
                       ...
                       (SELECT STRING_AGG(N'/* --- Running @Command against database ' + QUOTENAME(DB.database_name,'''') + N'. --- */' + @CRLF + @CRLF +
                                         ...
                                         END + @CRLF +
                                         N'END CATCH;' + @CRLF +
                                         N'/* --- Completed @Command against database ' + QUOTENAME(DB.database_name,'''') + N'. --- */',@Delimiter)
                        FROM @DBs DB) + @CRLF + @CRLF +
